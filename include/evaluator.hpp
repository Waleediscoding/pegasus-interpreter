...
class Evaluator : public Visitor, public StmtVisitor
{
  ...
  private:
    ...
    void visitKeywordStmt(KeywordStmt& stmt) override { stmt.m_keyword->accept(*this); }

    void visitExprStmt(ExprStmt& stmt) override { getExprValue(stmt); }
    ...
    void visitAssignExpr(Assignment& expr) override
    {
        std::unique_ptr<Binary> temp = std::make_unique<Binary>(
            std::make_unique<Literal>(m_env->value(expr.m_name)), expr.m_op,
    ...
    void visitGroupingExpr(Grouping& expr) override { m_result = eval(expr.m_expression.get()); }
    ...
    void visitLiteralExpr(Literal& expr) override { m_result = expr.m_value; }
  ...
  public:
    Evaluator() : globals{std::make_shared<Environment>()}
    {
        m_env = globals;
        // Native function
