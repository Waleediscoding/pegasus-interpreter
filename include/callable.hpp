...
class Callable
{
  public:
    ...
    Callable(std::string callee) : m_callee{callee} {}
    ...
    const std::string toString() { return std::string(std::format("<Function {}>", m_callee)); }
};

class Clock : public Callable
{
  public:
    Clock(std::string callee) : Callable(callee) {}
...
class Function : public Callable
{
  public:
    ...
    int arity() override { return m_declaration->m_params.size(); }
};